...
export function ComplexFormNew() {
  ...
  const [complexImages, setComplexImages] = useState<ComplexImageFile[]>([]);
                ...
                for (const file of files) {
                  ...
                  try {
                    ...
                    if (
                      ...
                      toast.error(
                        `${file.name}: image is too small (${dimensions.width}x${dimensions.height}). Minimum is ${MIN_COMPLEX_IMAGE_WIDTH}x${MIN_COMPLEX_IMAGE_HEIGHT}`
                      );
                      continue;
                    }
                    valid.push({
                    ...
                    toast.error(`${file.name}: image could not be read`);
                  }
                }
                setComplexImages((prev) => {
                  if (prev.length >= 20) {
                    toast.error('Maximum 20 images allowed');
                    return prev;
                  }
                  const slotsLeft = 20 - prev.length;
                  if (valid.length > slotsLeft) {
                    toast.error(`Only ${slotsLeft} image(s) were added (max 20)`);
                  }
                  return [...prev, ...valid.slice(0, slotsLeft)];
                });
                  ...
                  >
                    ...
                    <div className="p-2">
                      ...
                      <p className="text-[11px] text-gray-500">
                        {item.width}x{item.height} � {(item.file.size / 1024 / 1024).toFixed(2)} MB
                      </p>
                    </div>
                    <button
                      type="button"
                      onClick={() => setComplexImages((prev) => prev.filter((_, j) => j !== i))}
                      className="w-full border-t border-gray-200 text-red-600 hover:text-red-800 text-xs py-1"
                      aria-label="Remove"
                    >
                      Remove
                    </button>
